...
def create_meta(template_path, diff_patch=None, iterations=5):
...
def main():
    ...
    final_img = create_meta(SRC_FN, diff_patch=diff_patch, iterations=NUM_ITERATIONS)

    cv2.imwrite(OUT_FN, final_img)
    print(f"Generated recursive image saved to {OUT_FN}")
    view_image(OUT_FN)
